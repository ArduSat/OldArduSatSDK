...
void ITG3200::init(unsigned int  address, OnboardCommLayer* __ocl) {
    _ocl = __ocl;
...
void ITG3200::writemem(uint8_t _addr, uint8_t _val) {
    _buff[0] = _addr;
    _buff[1] = _val;
    _ocl->send((uint8_t*)&_buff[0], 2, _dev_address);

}

void ITG3200::readmem(uint8_t _addr, uint8_t _nbytes, uint8_t __buff[]) {
  _ocl->send((uint8_t*)&_addr, 1, _dev_address);
  _ocl->requestDataFrom(_dev_address, (int)_nbytes, &__buff[0], (int)_nbytes);
}
